...
WW_EXC_DEFAULT_PARAMS = {'a': 0.31, 'b': 0.125, 'd': 160.0, 'tau': 100.0, 'gamma': 0.641, 'W': 1.0, 'exc_current': 0.382, 'J_NMDA': 0.15, 'J_I': 1.0, 'lambda': LAMBDA_SPEED}
WW_INH_DEFAULT_PARAMS = {'a': 0.615, 'b': 0.177, 'd': 87.0, 'tau': 10.0, 'W': 0.7, 'exc_current': 0.382, 'J_NMDA': 0.15, 'lambda': LAMBDA_SPEED}
WW_REDUCED_DEFAULT_PARAMS = {'a': 0.27, 'b': 0.108, 'd': 154.0, 'tau': 100.0, 'gamma': 0.641, 'w': 0.6, 'J': 0.2609, 'exc_current': 0.33, 'lambda': LAMBDA_SPEED}
...
class ExcitatoryWongWangMass(WongWangMass):
    ...
    required_params = ['a', 'b', 'd', 'tau', 'gamma', 'W', 'exc_current', 'J_NMDA', 'J_I', 'lambda']
...
class InhibitoryWongWangMass(WongWangMass):
    ...
    required_params = ['a', 'b', 'd', 'tau', 'W', 'exc_current', 'J_NMDA', 'lambda']
...
class ReducedWongWangMass(WongWangMass):
    ...
    required_params = ['a', 'b', 'd', 'tau', 'gamma', 'w', 'J', 'exc_current', 'lambda']
...
class WongWangNode(SingleCouplingExcitatoryInhibitoryNode):
    ...
    def __init__(self, exc_params=None, inh_params=None, connectivity=WW_NODE_DEFAULT_CONNECTIVITY, exc_seed=None, inh_seed=None):
        """
        ...
        super().__init__(neural_masses=[excitatory_mass, inhibitory_mass], local_connectivity=connectivity, local_delays=None)
...
class WongWangNetwork(Network):
    ...
    def __init__(self, connectivity_matrix, delay_matrix, exc_mass_params=None, inh_mass_params=None, local_connectivity=WW_NODE_DEFAULT_CONNECTIVITY, exc_seed=None, inh_seed=None):
        """
        ...
        nodes = []
        for i, (exc_params, inh_params, local_conn) in enumerate(zip(exc_mass_params, inh_mass_params, local_connectivity)):
            node = WongWangNode(exc_params=exc_params, inh_params=inh_params, connectivity=local_conn, exc_seed=exc_seeds[i], inh_seed=inh_seeds[i])
            node.index = i
