...
class WilsonCowanNode(SingleCouplingExcitatoryInhibitoryNode):
    ...
    def __init__(self, exc_params=None, inh_params=None, connectivity=WC_NODE_DEFAULT_CONNECTIVITY, exc_seed=None, inh_seed=None):
        """
...
class WilsonCowanNetwork(Network):
    ...
    def __init__(self, connectivity_matrix, delay_matrix, exc_mass_params=None, inh_mass_params=None, local_connectivity=WC_NODE_DEFAULT_CONNECTIVITY, exc_seed=None, inh_seed=None):
        """
        ...
        for i, (exc_params, inh_params, local_conn) in enumerate(zip(exc_mass_params, inh_mass_params, local_connectivity)):
            node = WilsonCowanNode(exc_params=exc_params, inh_params=inh_params, connectivity=local_conn, exc_seed=exc_seeds[i], inh_seed=inh_seeds[i])
            node.index = i
